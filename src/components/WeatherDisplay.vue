<template>
  <div>
    <h2>Current: {{ weatherData.list[0].main.temp }}°C</h2>
    <img :src="iconUrl(weatherData.list[0].weather[0].icon)" />

    <h3>Next 6 Hours</h3>
    <div class="forecast-row">
      <div v-for="(hour, i) in weatherData.list.slice(0, 6)" :key="i">
    ...
    <h3>Next 5 Days</h3>
    <div class="forecast-row">
      <div v-for="(day, i) in weatherData.list.slice(0, 5)" :key="i">
        <p>{{ formatDay(day.dt) }}</p>
        <img :src="iconUrl(day.weather[0].icon)" />
        <p>{{ day.main.temp.day }}°C</p>
      </div>
    </div>
  </div>
</template>
